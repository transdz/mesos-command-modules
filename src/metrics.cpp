...
Metrics::Metrics(const MyAllocator& _allocator)
  : allocator(_allocator.self()),
    event_queue_dispatches(
        "allocator/mesos/event_queue_dispatches",
        process::defer(
            allocator, &MyAllocator::_event_queue_dispatches)),
...
void Metrics::addRole(const string& role)
{
  ...
  PullGauge gauge(
      ...
      defer(allocator,
            &MyAllocator::_offer_filters_active,
            role));
